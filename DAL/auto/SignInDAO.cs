...
public partial class SignInDAO{
#region 向数据库中添加一条记录 +bool Insert(SignIn model)
///<summary>
///向数据库中添加一条记录
///</summary>
///<param name="model">要添加的实体</param>
public bool Insert(SignIn model){
const string sql = @"INSERT INTO [dbo].[SignIn] (objectId,sustain,total,updateAt) VALUES (@objectId,@sustain,@total,@updateAt)";
int res = SqlHelper.ExecuteNonQuery(sql,new SqlParameter("@objectId",model.objectId.ToDBValue()),new SqlParameter("@sustain",model.sustain.ToDBValue()),new SqlParameter("@total",model.total.ToDBValue()),new SqlParameter("@updateAt",model.updateAt.ToDBValue()));
return res >0;
 }
#endregion
...
{
const string sql = "DELETE FROM [dbo].[SignIn] WHERE [objectId] = @objectId";
return SqlHelper.ExecuteNonQuery(sql, new SqlParameter("@objectId", objectId))>0;
}
#endregion
#region 根据主键ID更新一条记录 +bool Update(SignIn model)
/// <summary>
/// 根据主键更新一条记录
/// </summary>
/// <param name="model">更新后的实体</param>
/// <returns>是否成功</returns>
public bool Update(SignIn model)
{
const string sql = @"UPDATE [dbo].[SignIn] SET  sustain=@sustain,total=@total,updateAt=@updateAt  WHERE [objectId] = @objectId";
return SqlHelper.ExecuteNonQuery(sql,new SqlParameter("@objectId",model.objectId.ToDBValue()),new SqlParameter("@sustain",model.sustain.ToDBValue()),new SqlParameter("@total",model.total.ToDBValue()),new SqlParameter("@updateAt",model.updateAt.ToDBValue()))>0;
}
 #endregion
...
string sql = "SELECT COUNT(1) from SignIn "+str;var res = SqlHelper.ExecuteScalar(sql, list.ToArray());
return res == null ? 0 : Convert.ToInt32(res);
}
#endregion
#region 查询单个模型实体 +SignIn QuerySingleById(string objectId)
/// <summary>
/// 查询单个模型实体
/// </summary>
/// <param name="id">objectId</param>);
/// <returns>实体</returns>);
public SignIn QuerySingleById(string objectId)
{
const string sql = "SELECT TOP 1 objectId,sustain,total,updateAt from SignIn WHERE [objectId] = @objectId";
using (var reader = SqlHelper.ExecuteReader(sql, new SqlParameter("@objectId", objectId)))
{
if (reader.HasRows){
reader.Read();
SignIn model = SqlHelper.MapEntity<SignIn>(reader);
return model;
}
else
{
...
string[] clumns = new String[] { "objectId","sustain","total","updateAt" };
string[] cls = columns.parseColumnsX(clumns,"SignIn", out li);
string sql = "SELECT TOP 1 "+string.Join(",", li["SignIn"])+" from SignIn WHERE [objectId] = @objectId";
using (var reader = SqlHelper.ExecuteReader(sql, new SqlParameter("@objectId", objectId)))
{
if (reader.HasRows)
{
...
///<returns>实体</returns>
public SignIn QuerySingleByWheres(object wheres)
{
...
string[] clumns = new String[] { "objectId","sustain","total","updateAt" };
string[] cls = columns.parseColumnsX(clumns,"SignIn", out li);
string sql = "SELECT TOP 1 "+string.Join(",", li["SignIn"])+" from SignIn"+where;
...
public IEnumerable<SignIn> QueryList(int index, int size, object wheres=null, string orderField="objectId", bool isDesc = true)
{
List<SqlParameter> list = null;
string where = wheres.parseWheres(out list);
orderField=string.IsNullOrEmpty(orderField) ? "objectId" : orderField;
var sql = SqlHelper.GenerateQuerySql("SignIn",new string[]{"objectId","sustain","total","updateAt"}, index, size, where, orderField, isDesc);
using (var reader = SqlHelper.ExecuteReader(sql,list.ToArray()))
{
if (reader.HasRows)
{
...
string[] clumns = new String[] { "objectId","sustain","total","updateAt" };
string[] cls = columns.parseColumnsX(clumns,"SignIn", out li);
var sql = SqlHelper.GenerateQuerySql("SignIn", li["SignIn"], index, size, where, orderField, isDesc);
using (var reader = SqlHelper.ExecuteReader(sql,list.ToArray()))
{
if (reader.HasRows)
{
...
string sql = string.Format(@"UPDATE [dbo].[SignIn] SET  {0}  WHERE [{1}] = @{1}", string.Join(",", column), "objectId");
...
list.AddRange(list1);
string sql = string.Format(@"UPDATE [dbo].[SignIn] SET  {0} {1}", string.Join(",", column), where);
return SqlHelper.ExecuteNonQuery(sql, list.ToArray()) > 0;
}
 #endregion
}
