...
const string sql = @"INSERT INTO [dbo].[authData] (objectId,weiboId,qqId,alibabaId,wechatId) VALUES (@objectId,@weiboId,@qqId,@alibabaId,@wechatId)";
int res = SqlHelper.ExecuteNonQuery(sql,new SqlParameter("@objectId",model.objectId.ToDBValue()),new SqlParameter("@weiboId",model.weibo.objectId.ToDBValue()),new SqlParameter("@qqId",model.qq.objectId.ToDBValue()),new SqlParameter("@alibabaId",model.alibaba.objectId.ToDBValue()), new SqlParameter("@wechatId", model.wechat.objectId.ToDBValue()));
return res >0;
 }
#endregion
...
{
const string sql = @"UPDATE [dbo].[authData] SET  weiboId=@weiboId,qqId=@qqId,alibabaId=@alibabaId,wechatId=@wechatId  WHERE [objectId] = @objectId";
return SqlHelper.ExecuteNonQuery(sql,new SqlParameter("@objectId",model.objectId.ToDBValue()),new SqlParameter("@weiboId",model.weibo.objectId.ToDBValue()),new SqlParameter("@qqId",model.qq.objectId.ToDBValue()),new SqlParameter("@alibabaId",model.alibaba.objectId.ToDBValue()), new SqlParameter("@wechatId", model.wechat.objectId.ToDBValue())) >0;
}
 #endregion
...
{
const string sql = "SELECT TOP 1 objectId,weiboId,qqId,alibabaId,wechatId from authData WHERE [objectId] = @objectId";
...
model.alibaba = alibabaDAO.QuerySingleById((string)reader["alibabaId"]);
                    ...
                    if (reader["wechatId"] != DBNull.Value)
                    {
                        wechatDAO wechatDAO = new wechatDAO();
                        model.wechat = wechatDAO.QuerySingleById((string)reader["wechatId"]);
                    }
                    return model;
}
else
...
model["alibaba"] = alibabaDAO.QuerySingleByIdX((string)reader["alibabaId"],li["alibaba"]);
 ...
 model["alibaba"]=null;
                    ...
                    if (li.ContainsKey("wechat"))
                    {
                        if (reader["wechatId"] != DBNull.Value)
                        {
                            wechatDAO wechatDAO = new wechatDAO();
                            model["wechat"] = wechatDAO.QuerySingleByIdX((string)reader["wechatId"], li["wechat"]);
                        }
                        else
                        {
                            model["wechat"] = null;
...
var sql = SqlHelper.GenerateQuerySql("authData",new string[]{"objectId","weiboId","qqId","alibabaId","wechatId"}, index, size, where, orderField, isDesc);
using (var reader = SqlHelper.ExecuteReader(sql,list.ToArray()))
{
if (reader.HasRows)
{
