...
namespace Sheep.Controllers
{
    public class CreditsDetailsController : baseApiController
    {
        ...
        public IHttpActionResult GetItems(string v1, int limit = 10, int page = 0, int count = 0, string where = null, string include = null, string order = null)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(include))
                {
                    ...
                    if (count == 1)
                    {
                        int total_num = bll.QueryCount(list);
                        return ok(new { total_num = total_num, results = models });
                    }
                    else
