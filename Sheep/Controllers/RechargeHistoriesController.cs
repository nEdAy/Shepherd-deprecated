...
{
    public class rechargeHistoriesController : baseApiController
    {
        RechargeHistoryBLL bll = new RechargeHistoryBLL();
        public IHttpActionResult GetrechargeHistory(String v1, string userId)
        {
            RechargeHistory model = new RechargeHistory();
            ...
            if (result) {
                ...
                //return create(model.objectId);
            }
            return notFound("订单生成失败");
        }
        public IHttpActionResult GetrechargeHistories(string v1, int limit = 10, int page = 0, string where = null, string include = null, string order = null)
        {
            try
            {
